...
{
    if (stream == NULL) {
        fprintf(stderr, "ERROR: stream is NULL in exp_local_variable_to_stream\n");
        return;
    }
    
    if (expression == NULL) {
        fprintf(stderr, "ERROR: expression is NULL in exp_local_variable_to_stream\n");
        return;
    }
    
    jd_val *val = expression->data;
    if (val == NULL) {
        fprintf(stderr, "ERROR: val data is NULL in exp_local_variable_to_stream\n");
        return;
    }
    
    if (val->name == NULL) {
        fprintf(stderr, "ERROR: val name is NULL in exp_local_variable_to_stream\n");
        return;
    }
